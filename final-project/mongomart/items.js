...
function ItemDAO(database) {
        ...
        */
        ...
        this.db.collection("item").aggregate(
          ...
          .toArray(function(err, results) {
            ...
            var category = categories.reduce(function(c1, c2) {
              return { "_id": "All", "num": c1.num + c2.num };
            })
            categories.push(category);
            categories.sort(function(c1, c2) {
              return c1["_id"].toLowerCase() < c2["_id"].toLowerCase() ? -1 : 1;
            })
            callback(categories);
          }
        )
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        this.db.collection("item").find(query).sort(["_id", 1])
               .skip(itemsPerPage * page).limit(itemsPerPage)
               .toArray(function(err, results) {
                 if (err) throw err;
    ...
    this.getNumItems = function(category, callback) {
        ...
        var query = category == "All" ? {} : { "category": category };
        this.db.collection("item").find(query).toArray(function(err, results) {
          if (err) throw err;
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        this.db.collection("item").find(searchQuery).sort(["_id", 1])
               .skip(itemsPerPage * page).limit(itemsPerPage)
               .toArray(function(err, results) {
                 if (err) throw err;
                 var items = results;
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        var searchQuery = { "$text": { "$search": query } };
        this.db.collection("item").find(searchQuery).toArray(function(err, results) {
          if (err) throw err;
          var numItems = results.length;
          callback(numItems);
        })
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection("item")
               .findOne({ "_id": itemId}, function(err, result) {
                 if (err) throw err;
                 var item = result;
                 callback(item);
               });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var searchQuery = { "_id": itemId };
        var updateQuery = { "$push": { "reviews": reviewDoc } };

        this.db.collection("item")
               .findOneAndUpdate(searchQuery, updateQuery, { "returnOriginal": false }, function(err, result) {
                 if (err) throw err;
